...
export default function Whiteboard() {
  ...
  const onPointerDown = (e) => {
    ...
    smoothedPressure.current = pres; // start from current value, no cold start
  ...
  const onPointerUp = async () => {
    ...
    // ── Save to Google Sheets if valid gold reading ──
    const feat       = featuresRef.current;
    const lastForce  = livePressure.current;
    const pct        = lastForce * 100;
    const ct = pct >= 22 && pct <= 30 ? "22ct"
             : pct >= 31 && pct <= 35 ? "18ct"
             : pct > 35               ? "Spurious"
             : null;
